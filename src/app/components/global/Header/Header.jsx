...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-slate-100/90 border-2 rounded-sm border-slate-300 backdrop-blur-sm">
      <div className="max-w-[1200px] mx-auto px-6 py-5 flex items-center justify-between gap-4">
        <Link href="/" className="flex items-center gap-3">
          <Image src="/logo.jpeg" alt="RBSH studio" width={90} height={32} priority className=" object-contain w-12 h-12 rounded-lg" />
        </Link>

        <nav className="hidden md:flex md:items-center md:gap-10">
          <Link href="/" className="text-gray-800 tracking-wider font-semibold  hover:text-orange-950">HOME</Link>
          <Link href="/about" className="text-gray-800 tracking-wider font-semibold hover:text-orange-950">ABOUT</Link>
          <Link href="/services" className="text-gray-800 tracking-wider font-semibold hover:text-orange-950">OUR SERVICES</Link>
          <Link href="/blog" className="text-gray-800 tracking-wider font-semibold hover:text-orange-950">BLOG</Link>
          <Link href="/careers" className="text-gray-800 tracking-wider font-semibold hover:text-orange-950">CAREER</Link>
        </nav>
      ...
      <div className={`md:hidden bg-brandbg border-t border-branddark/60 overflow-y-auto transition-[max-height] duration-300 ${open ? 'max-h-[80vh]' : 'max-h-0'}`}>
        <div className="px-6 py-4 flex flex-col gap-3">
          ...
          <Link href="/services" className="block text-gray-800 font-semibold">OUR SERVICES</Link>
